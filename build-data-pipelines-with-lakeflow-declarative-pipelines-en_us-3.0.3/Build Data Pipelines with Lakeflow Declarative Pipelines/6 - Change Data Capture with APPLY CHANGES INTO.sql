...
-- MAGIC %md
-- MAGIC
-- MAGIC **NOTE**: If the pop-up Run in strict sandbox appears on the Pipeline page, click on **Run in strict sandbox** to proceed.
-- MAGIC
-- MAGIC
...
-- MAGIC
-- MAGIC    d. In the new tab you should see four folders: **explorations**, **orders**, **status** and **customers**.(Plus the extra python_excluded folder that contains the Python version). 
-- MAGIC
...
-- MAGIC
-- MAGIC    **WARNING:** If you get the following warning when opening the **customers_pipeline** file: 
-- MAGIC
