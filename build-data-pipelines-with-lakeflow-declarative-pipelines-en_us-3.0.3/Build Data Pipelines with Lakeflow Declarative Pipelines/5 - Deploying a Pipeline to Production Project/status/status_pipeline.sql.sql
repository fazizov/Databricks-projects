...
-- MAGIC
-- MAGIC 3. Creates the materialized view **full_order_info_gold_demo5** to capture each order's status by joining the following tables:
...
-- MAGIC
-- MAGIC    c. The **full_order_info_gold_demo5** materialized view join contains a total of 946 rows (the previous 536 rows + the new 410 rows).
-- MAGIC
-- MAGIC    d. The **cancelled_orders_gold_demo5** materialized view contains 21 rows.
-- MAGIC
-- MAGIC    e. The **delivered_orders_gold_demo5** materialized view contains 176 rows.
-- MAGIC
-- MAGIC    f. The **gold_orders_by_date_demo5** materialized view contains 11 rows.
