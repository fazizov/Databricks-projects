...
# MAGIC ### You've chosen the version where you will need to fill in applyInPandasWithState code.  If that is not what you want to do, please choose notebook 01 instead
...
# MAGIC #### TO-DO - fill in the missing code
# MAGIC * You are using directory-listing mode
# MAGIC * The incoming files are JSON format
# MAGIC * You want to enable schema evolution to allow it to add new columns
# MAGIC * You want to enable schema inference
# MAGIC * You want to include existing files
...
# MAGIC #### TO-DO - fill in the missing code
# MAGIC * You are using directory-listing mode
# MAGIC * The incoming files are CSV format, and there is a header
# MAGIC * You want to enable schema evolution to allow it to add new columns
# MAGIC * You want to enable schema inference
# MAGIC * You want to include existing files
...
# MAGIC #### TO-DO - fill in the missing code
# MAGIC * Define a change flow
# MAGIC * The target is the streaming table that was just created in the cell
# MAGIC * The source is the raw bets table that was populated in one of the cells above
# MAGIC * The key being updated on is the <b>customerId</b>
# MAGIC * The bets are sequenced by <b>eventTimestamp</b>
# MAGIC * This table is only keeping the latest record for each key
...
# MAGIC #### TO-DO - call applyInPandasWithState
# MAGIC * Define a 5 second watermark on the <b>eventTimestamp</b> column
# MAGIC * Group by (hint - groupBy) the <b>customerId</b> column
# MAGIC * The function that is being called by applyInPandasWithState is named <b>updateState</b>.  You can scroll down a couple of cells to see the logic for it.
# MAGIC * The output schema is in a variable called <b>outputSchema</b>
# MAGIC * The schema for the state is in a variable called <b>stateSchema</b>
# MAGIC * Use append mode
# MAGIC * Event time-based timeouts are being used (hint - you must include <b>GroupStateTimeout.EventTimeTimeout</b> as part of the applyInPandasWithState signature)
...
def stream_from_raw():
  ...
  # Calculate the count and amount of bets over time with applyInPandasWithState
  applyInPandasWithStateResultDf = (
    rawBetsDf
    # TO-DO
    .applyInPandasWithState(# TO-DO)
  )
...
# MAGIC #### TO-DO - Define the materialized view
# MAGIC * Do a batch left-join of the <b>betsLatestTableName</b> table and the <b>customerInfoTableName</b> table on <b>customerId</b>
# MAGIC * Hint - do you know the difference between dlt.read and dlt.readStream?
...
# MAGIC #### TO-DO - fill in the missing code
# MAGIC * There are multiple sections of code missing in the updateState function below.  Complete the logic so that it satisfies the requirements listed above and runs successfully in the pipeline
# MAGIC * Search for <b>TO-DO</b> below to find the sections of missing code
...
# The schema for what will be stored in state - a list of tuples with the bet stake (stake is the bet amount), the event timestamp (when the bet was generated), the emit timestamp (when the bet's aggregation should be calculated and removed) and the process timestamp (when the bet entered the system)
...
def updateState (
  ...
  if not state.hasTimedOut:
    ...
    if state.exists:
      # TO-DO - GET THE STATE.  Remember the state is a tuple
    ...
    if len(prevBetList) > 0:
      ...
      for bet in betsInState:
        ...
        if bet.emitTimestamp <= currentWatermarkTimestamp:
          currentTimestamp = # TO-DO - get the current timestamp
          prevBetList.remove(bet)
          betCount = # TO-DO - decrement the bet count
          totalBetsAmount = # TO-DO - decrement the total bets amount.  You will need to check the structure of a bet record above
    ...
    for bet in newBetList:
      currentTimestamp = # TO-DO - get the current timestamp
      betCount = # TO-DO - increment the bet count
      totalBetsAmount = # TO-DO - increment the total bets amount.  You will need to check the structure of a bet record above
      durationInState = (currentTimestamp - bet.processTimestamp).total_seconds()
  ...
  # We set the timeout to fire for the time when the next bet in state was supposed to be emitted
    ...
    isTimeout = True
    # TO-DO - get the current state and the key value
    ...
    for bet in betsInState:
      ...
      if bet.emitTimestamp <= currentWatermarkTimestamp:
        ...
        prevBetList.remove(bet)
        betCount = # TO-DO - decrement the bet count
        totalBetsAmount = # TO-DO - decrement the total bet amount
        durationInState = (currentTimestamp - bet.processTimestamp).total_seconds()
        aggregateRecord = AggregationRecord(betCount, totalBetsAmount, bet.eventTimestamp, bet.emitTimestamp, bet.processTimestamp, durationInState, "subtract")
        aggregationRecordList.append(aggregateRecord)

    # Is the betCount now 0?  If so remove the entire state and do not set a timeout.
    # Note the parentheses are required in the remove() call, otherwise the state is not removed and the existing timeout will fire over and over
    if betCount == 0:
      # TO-DO - remove the state
