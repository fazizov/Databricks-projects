...
# MAGIC |Cluster Policies| **DBAcademy** for clusters running standard notebooks <br> **DBAcademy Jobs** for workflows/jobs <br> **DBAcademy DLT** for DLT pipelines <br> **DBAcademy DLT-UC** for DLT pipelines with Unity Catalog|
...
dbutils.widgets.text(WorkspaceHelper.PARAM_SPARK_VERSION, "15.4.x-scala2.12", "Spark Version (optional)")
...
# MAGIC %md
# MAGIC Create a fourth policy like DLT but with one worker node for use with UC. This is currently a product requirement and the helper functions don't allow for this.

# COMMAND ----------

import json

dlt_policy_name = 'DBAcademy DLT'
dlt_uc_policy_def = json.loads(DA.client.cluster_policies.get_by_name(dlt_policy_name).get('definition'))
dlt_uc_policy_def['num_workers'] = {
    "type": "range",
    "maxValue": 1
}

del(dlt_uc_policy_def['spark_conf.spark.databricks.cluster.profile'])

dlt_uc_policy_id = DA.client.cluster_policies.create_or_update(f'{dlt_policy_name}-UC', dlt_uc_policy_def).get('policy_id')
DA.client.permissions.cluster_policies.update_group(dlt_uc_policy_id, 'users', 'CAN_USE')
...
# MAGIC Creates a single warehouse to be used by all students.
# MAGIC
